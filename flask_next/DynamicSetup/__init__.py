...
class DynamicSetup():
                    ...
                    for param in sig.parameters.values():
                        ...
                        if param.annotation is not inspect.Parameter.empty:
                            if param.annotation is int:
                                ...
                                self.app.logger.warning(f'{method_name}: unknown parameter type: {param.annotation}')
                        rule_url += f"/<{param_type}{param.name}>"

                    self.app.logger.info(f'{method_name}: add_url_rule: {rule_url} {methods}')
                    endpoint = f'{module_name}.{method_name}'
                    self.app.add_url_rule(rule_url, view_func=view_func,
    ...
    def add_html(self, route_path: Path, rule_url: str):
        ...
        self.app.add_url_rule(rule_url, view_func=HTMLView.as_view(endpoint, route_path), endpoint=endpoint)
        self.route_count += 1
    ...
    def print(self):
        ...
        self.app.logger.inot(self.secure_routes)